...
-- Dosya sistemi için
local VORTEX_FOLDER = "VortexHelper"
local USED_SERVERS_FILE = "used_servers.json"

-- Tablo uzunluğu için yardımcı fonksiyon
...
local function loadUsedServers()
    local success, result = pcall(function()
        if not isfolder(VORTEX_FOLDER) then
            makefolder(VORTEX_FOLDER)
            return {}
        end
        
        if not isfile(VORTEX_FOLDER .. "/" .. USED_SERVERS_FILE) then
            writefile(VORTEX_FOLDER .. "/" .. USED_SERVERS_FILE, "{}")
            return {}
        end
        
        local fileContent = readfile(VORTEX_FOLDER .. "/" .. USED_SERVERS_FILE)
    ...
    if success then
        ...
        print("❌ File load error: " .. tostring(result))
        return {}
    end
end
...
local function saveUsedServers()
    local success, error = pcall(function()
        ...
        local jsonData = HttpService:JSONEncode(usedServerIds)
        writefile(VORTEX_FOLDER .. "/" .. USED_SERVERS_FILE, jsonData)
    end)
    
    if not success then
        print("❌ File save failed: " .. tostring(error))
    end
end

-- DataStore'u başlat
usedServerIds = loadUsedServers()
...
local function showScanningEffect()
    ...
    scanningGui.Name = "ScanningEffect"
    scanningGui.ResetOnSpawn = false
    scanningGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    scanningGui.Parent = CoreGui
    
    local scanningFrame = Instance.new("Frame")
    scanningFrame.Size = UDim2.new(0, 300, 0, 100)
    scanningFrame.Position = UDim2.new(0.5, -150, 0.4, -50)
    scanningFrame.BackgroundColor3 = Color3.fromRGB(10, 10, 20)
    scanningFrame.BackgroundTransparency = 0.1
    scanningFrame.BorderSizePixel = 0
    scanningFrame.ZIndex = 999
    scanningFrame.Parent = scanningGui
    
    local corner = Instance.new("UICorner")
    corner.CornerRadius = UDim.new(0, 12)
    corner.Parent = scanningFrame
    
    local stroke = Instance.new("UIStroke")
    stroke.Color = Color3.fromRGB(0, 150, 255)
    stroke.Thickness = 2
    stroke.Parent = scanningFrame
    
    -- Scanning text
    local scanText = Instance.new("TextLabel")
    scanText.Size = UDim2.new(1, 0, 0, 30)
    scanText.Position = UDim2.new(0, 0, 0, 10)
    scanText.BackgroundTransparency = 1
    scanText.Text = "🔍 SCANNING SERVERS"
    scanText.TextColor3 = Color3.fromRGB(0, 200, 255)
    scanText.TextSize = 16
    scanText.Font = Enum.Font.GothamBold
    scanText.ZIndex = 1000
    scanText.Parent = scanningFrame
    ...
    progressBar.Parent = scanningFrame
    ...
    if result and result.data then
        ...
        if #availableServers > 0 then
            table.sort(availableServers, function(a, b)
                return math.abs(a.players - 25) < math.abs(b.players - 25)
            end)
            
            local targetServer = availableServers[1]
            
            if scanningEffect then scanningEffect:Destroy() end
            showNotification("🚀 Joining new server (" .. targetServer.players .. " players)", true)
...
local function getSystemInfo()
    ...
    table.insert(info, "🔍 Scan Effect v2")
...
mainFrame.Position = UDim2.new(0.5, -70, 0.5, -65)
...
print("⚙️ Version 1.8 - Scan Effect")
print("📁 Folder: workspace/VortexHelper/")
print("💾 Tracking " .. countTable(usedServerIds) .. " servers")
print("🔍 Scanning Effect: ACTIVE")
